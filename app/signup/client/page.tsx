  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: origin ? `${origin}/dashboard/client` : undefined,
          data: {
            full_name: fullName,
            signup_as: "client",
          },
      ...
      if (!session) {
        toast.info("Check your email to confirm your account, then sign in to continue.")
        router.push("/login?next=/dashboard/client&registered=1")
        return
      }
      ...
      <header className="border-b border-border">
        <div className="mx-auto flex h-16 max-w-[1400px] items-center justify-between px-4 md:px-8">
          ...
          <div className="flex items-center gap-4 text-sm">
            <span className="hidden text-foreground/80 md:inline">Here to find work?</span>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3 pt-2">
            <label className="flex items-start gap-3 text-sm">
              ...
              <span>Send me helpful emails to find and hire great talent.</span>
            </label>
            <label className="flex items-start gap-3 text-sm">
